...
local status_ok, tasks_nv = pcall(require, 'tasks')
...
tasks_nv.setup({
  default_params = { -- Default module parameters with which `neovim.json` will be created.
    cmake = {
      ...
      build_dir = tostring(Path:new('{cwd}', 'build', '{os}-{build_type}')), -- Build directory. The expressions `{cwd}`, `{os}`
      ...
      dap_name = 'cppdbg', -- DAP configuration name from `require('dap').configurations`.
      -- If there is no such configuration, a new one with this name as `type` will be created.
  ...
  params_file = 'neovim_tasks.json', -- JSON file to store module and task parameters.
  quickfix = {
